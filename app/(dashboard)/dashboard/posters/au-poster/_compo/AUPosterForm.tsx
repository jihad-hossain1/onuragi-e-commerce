...
import { posterServerAction } from "./serverAction";
...
const AUPosterForm = ({ posterData, id, products }) => {
  ...
  React.useEffect(() => {
    if (posterData) {
      setFormData({
        ...
        productId: posterData?.productId,
      });
      setPhoto(posterData?.image);
    }
  }, [posterData]);
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (id) {
        ...
        if (data?.result) {
          setLoading(false);
          toast("Poster Updated Successfull");
          validatedTag("poster");
          router.refresh();
        }
      } else {
        const data = await posterServerAction({
          title: formData?.title,
          image: _photo,
          productId: formData?.productId,
        });
        ...
        if (data?.result) {
          setLoading(false);
          toast("Poster Added Successfull");
          validatedTag("poster");
          router.refresh();
        }
      }
  ...
  return (
    <main className="">
      <Link href="/dashboard/posters" className="btn w-fit">
        Back
      </Link>
